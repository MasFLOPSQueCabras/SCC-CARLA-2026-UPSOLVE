...
1. SCC_CLUSTER_MANIFEST / SCC_MANIFEST environment variable
2. CLI argument --manifest <path>
3. values.yaml in current working directory or repository root
4. Named cluster config in configs/clusters/${SCC_CLUSTER}.yaml
...
def _locate_manifest_path(explicit_manifest: str | None = None) -> Path | None:
    ...
    env_manifest = os.environ.get("SCC_CLUSTER_MANIFEST") or os.environ.get(
        "SCC_MANIFEST"
    )
    if env_manifest:
        p = Path(env_manifest)
    ...
    cluster_name = os.environ.get("SCC_CLUSTER")
    if cluster_name:
...
def load_cluster_manifest(manifest_path: Path) -> dict[str, Any]:
    repo_root = _find_repo_root()
    core_src = repo_root / "packages" / "scc-core" / "src"
...
def build_inventory(manifest_data: dict[str, Any]) -> dict[str, Any]:
    ...
    team_id_raw = os.environ.get("SCC_TEAM_ID", "72")
    try:
    ...
    domain = network.get("domain", "scc.local")
