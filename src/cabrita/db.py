...
from cabrita.paths import get_local_db_path
from cabrita.state_worker import execute_action, get_file_hash
...
def _ensure_worker_synced(settings: ClusterSettings) -> None:
    ...
    with _DB_THREAD_LOCK:
        ...
        remote_dir = Path.home() / ".config" / "cabrita"
        ...
        check_cmd = [
            ...
            "ControlMaster=auto",
            "-o",
            "ControlPath=/tmp/cabrita-ssh-%r@%h:%p",
            "-o",
            "ControlPersist=60s",
...
def _run_bastion_ssh_action(
    ...
    cmd = [
        ...
        "ControlPersist=60s",
        settings.bastion_ssh_host,
        "python3 ~/.config/cabrita/state_worker.py",
    ]
...
def _dispatch_db_action(
    ...
    with _DB_THREAD_LOCK:
        if (
            ...
            or os.environ.get("CABRITA_LOCAL_DB") == "1"
        ):
            local_db_path = (
