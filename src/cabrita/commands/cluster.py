...
def init(
    ...
    try:
        ref = ir.files("cabrita").joinpath("ansible")
    ...
    if no_provider:
        console.print(
            ...
            "Run [bold]cabrita provider add <helvetios|libvirt>[/bold] when ready to configure a provider."
        )
        return
    ...
    if chosen_prov is None:
        if sys.stdin.isatty():
            ...
            if choice in ("1", "helvetios", "bmc"):
                ...
                console.print(
                    f"\n[bold green]Base workspace initialized successfully in {target_dir}![/bold green]\n"
                    "Run [bold]cabrita provider add <helvetios|libvirt>[/bold] when ready."
                )
                return
        else:
            chosen_prov = "vm"
...
def show(
    ...
    if not path.exists():
        console.print(
            ...
            "[dim]Run 'cabrita init' or 'cabrita provider add <provider>' to initialize a workspace.[/dim]"
        )
        raise typer.Exit(code=1)
