...
from scc_core.di import ProviderNotInstalledError, container
...
def customize_helvetios_yaml(
    ...
    if bastion_ssh_host is not None:
        yaml_text = yaml_text.replace(
            "ssh_host: scc-bastion", f"ssh_host: {bastion_ssh_host}"
        )
...
def add_provider_cli(
    ...
    console.print(
        ...
        f"You can now run [bold]scc cluster show[/bold], [bold]scc plan[/bold], or [bold]scc up[/bold]."
    )
